...
class DigitsDataset(Dataset):
    def __init__(self, digit_imgs_X, target_labels_y, transform=None):
        ...
        # # Ensure data is sorted by target
        # indices = np.argsort(target_labels_y)

        # pprint(indices)
        # pprint(self.digit_imgs_X[indices[0]])
        # pprint(self.target_labels_y[indices][0])

        # self.digit_imgs_X = self.digit_imgs_X[indices]
        # self.target_labels_y = self.target_labels_y[indices]

    # def __len__(self): return len(self.digit_imgs_X)
    def __len__(self):
        max_length = max(
            len(label_indicies)
